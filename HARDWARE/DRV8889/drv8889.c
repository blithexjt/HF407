...
void DRV8889_Init()
{
	...
	GPIO_InitStucture.Pin = DIR_pin|Dir_Y_pin;
	GPIO_InitStucture.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStucture.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOD,&GPIO_InitStucture);

	GPIO_InitStucture.Pin = nSleep_pin|nSleep_Y_pin;
	GPIO_InitStucture.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStucture.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOC,&GPIO_InitStucture);
	
	GPIO_InitStucture.Pin = nFault_X_pin|nFault_Y_pin;
	GPIO_InitStucture.Mode = GPIO_MODE_INPUT;
	GPIO_InitStucture.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(GPIOD, &GPIO_InitStucture);

	GPIO_InitStucture.Pin = Drvoff_pin|CS_pin|Drvoff_Y_pin;
	GPIO_InitStucture.Mode = GPIO_MODE_OUTPUT_OD;
	GPIO_InitStucture.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOD,&GPIO_InitStucture);

	GPIO_InitStucture.Pin = CS_Y_pin;
	GPIO_InitStucture.Mode = GPIO_MODE_OUTPUT_OD;
	GPIO_InitStucture.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOG,&GPIO_InitStucture);
...
void Drv8889_ToffTime(u8 Motor,TOFF_time OFFTime)
{
	u8 temp1;
	SLEW_RATE temp2 = OFFTime<<3;
